...
import type { ConsultJorfResponse, GetJorfContResponse, GetJosResponse } from './types';
...
export async function listLastNJo(n: number = 5): Promise<GetJorfContResponse | null> {
...
export async function getJoSummary(textCid: string): Promise<GetJosResponse | null> {
	const BEARER = await getAccessToken();
...
export async function getJoDetail(textCid: string): Promise<ConsultJorfResponse | null> {
	const BEARER = await getAccessToken();
