...
import { getJoDetail, getJoSummary, listLastNJo } from './scraper';
import type { ConsultJorfResponse, GetJosResponse, Lien, Tm } from './scraper/types';
import { wrappedLanguageModel } from './ai';
import { z } from 'zod';
			...
			tms: (
				| {
						...
						tms: (
							...
							| {
									titre: "Ministère de l'éducation nationale, de l'enseignement supérieur et de la recherche";
									liensTxt: Lien[];
							  }
							| {
									titre: 'Ministère du travail, de la santé, des solidarités et des familles';
									liensTxt: Lien[];
							  }
							| {
...
function renderJoToMarkdownSubForTableOfContents(originalTms: Tm[] | undefined, date: string, index: number = 0): string {
...
function renderJoToMarkdownSub(
	...
	const elements = originalTms.map((e, mapIndex) => {
		const title = `${'#'.repeat(index + 1)} ${e.titre}` + (index == 0 ? ` (${date})` : '');
...
function getAllLienIdToFetch(originalTms: Tm[] | undefined): string[] {
	...
	const liensForTms = tmsForThisLevel ? getAllLienIdToFetch(tmsForThisLevel) : [];
	const liensTxtForThisLevel = originalTms?.flatMap((e) => e.liensTxt);
	const allLienIds = [...liensForTms, ...(liensTxtForThisLevel?.map((e) => e.id) ?? [])];
	console.log('allLienIds', allLienIds);
	return allLienIds;
}

async function fetchAllLiens(originalTms: Tm[] | undefined): Promise<Record<string, ConsultJorfResponse>> {
...
async function renderJoToMarkdown(joSummaryResponse: GetJosResponse, date: string) {
	if (!joSummaryResponse?.items || joSummaryResponse.items.length === 0) {
		return 'No items found in the JO summary. 🚫';
	}

	const journalOfficiel = joSummaryResponse.items[0]?.joCont?.structure.tms.find((e) => e.titre === 'Journal officiel "Lois et Décrets"');
...
export async function getTweetForLastJo() {
	...
	const firstContainerDate = new Date(firstContainer.datePubli).toISOString().split('T')[0];
...
`.trim();
	...
	return {
		...
		url: `https://www.legifrance.gouv.fr/jorf/jo/${year}/${month.toString().padStart(2, '0')}/${day.toString().padStart(2, '0')}/${
			container.num
		}`,
		object: resAi.object,
		// format DD/MM/YYYY
